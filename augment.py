...
def augment_data(data0, data1, truth0, truth1, affine0, affine1, scale_deviation=None, flip=True):
    n_dim = len(truth0.shape)
    if scale_deviation:
    ...
    data_list0 = list()
    for data_index in range(data0.shape[0]):
        image = get_image(data0[data_index], affine0)
        data_list0.append(resample_to_img(distort_image(image, flip_axis=flip_axis,
                                                       scale_factor=scale_factor), image,
                                         interpolation="continuous").get_data())
    data0 = np.asarray(data_list0)
    truth_image0 = get_image(truth0, affine0)
    truth_data0 = resample_to_img(distort_image(truth_image0, flip_axis=flip_axis, scale_factor=scale_factor),
                                 truth_image0, interpolation="nearest").get_data()

    data_list1 = list()
    for data_index in range(data1.shape[0]):
        image = get_image(data1[data_index], affine1)
        data_list1.append(resample_to_img(distort_image(image, flip_axis=flip_axis,
                                                       scale_factor=scale_factor), image,
                                         interpolation="continuous").get_data())
    data1 = np.asarray(data_list1)
    truth_image1 = get_image(truth1, affine1)
    truth_data1 = resample_to_img(distort_image(truth_image1, flip_axis=flip_axis, scale_factor=scale_factor),
                                 truth_image1, interpolation="nearest").get_data()

    return data0, truth_data0, data1, truth_data1
